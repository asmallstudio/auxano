...
function NavListItems({ siteData, ...props }) {
  return (
    <>
...
function Header({ siteData }) {
  ...
  if (typeof window !== "undefined") {
    initialScreenState = window.matchMedia("(min-width: 64em)").matches
      ? "wide"
      : "narrow";
  }

  const [menuHidden, setMenuHidden] = React.useState(true);
  const [menuPersistent, setMenuPersistent] = React.useState(false);
  const [screenSize, setScreenSize] = React.useState(initialScreenState);

  React.useEffect(() => {
    // On mounting of component, make sure Menu Persistence is set appropriately.
    checkMenuPersistence();
    ...
    <header className={styles.header}>
      <div className={styles.headerContainer}>
        ...
        <nav id="navigation" className={styles.nav}>
          <MenuIcon menuHidden={menuHidden} menuAction={_menuToggle} />
          <ul
            id="header-nav"
          ...
          >
            <NavListItems siteData={siteData} onClick={_menuClose} />
...
function MenuIcon({ menuHidden, menuAction }) {
  let menuIcon;
